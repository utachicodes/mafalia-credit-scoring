  ...
  return (
    <div className="border rounded-lg p-4">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-sm text-muted-foreground">{t("dashboard.configurableScore.title")}</div>
          <div className="text-2xl font-semibold">{score100} / 100</div>
        </div>
        <Link className="underline text-sm" href="/scoring">
          {t("dashboard.configurableScore.adjustLink")}
        </Link>
      </div>
      <div className="text-xs text-muted-foreground mt-2">
        {t("dashboard.configurableScore.description", { client: clientId })}
      </div>
    </div>
  )
}
